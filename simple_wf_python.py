...
#@constraint(computing_units="48")
#@task(on_failure = 'IGNORE')
#def run_haddock(input_run1_air_dir, pdb_dir, haddock_img):
    print(os.getcwd())
    input_run1_air_dir = os.getenv('TMPDIR') + '/' + pdb_dir + '/run1-ranair' 
    #print('AFFINITY for proc runhaddock' + process_name + ' ' + str(os.getpid()) +': ' + str(len(os.sched_getaffinity(os.getpid()))))
    ...
    print(" ".join(singularity_cmd))
    #print("Ambiente: ", dict(os.environ))
...
if __name__ == '__main__':
    ...
    haddock_img = "/home/bsc19/bsc19275/haddock/code/run4/BM5-clean/haddock24_v1.2.sif"
    ...
    for pdb_dir in os.listdir(pdbs_dir):
        ...
        #run_param_path = compss_wait_on(run_param_path)
        #scratch_dir = os.getenv('TMPDIR') + '/' + pdb_dir + '/run1-ranair'
        #run_haddock(scratch_dir, pdb_dir, haddock_img)
        i+=1
        if (i==49):
            break
